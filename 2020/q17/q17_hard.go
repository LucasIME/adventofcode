...
func toKey(pos []int) string {
	return fmt.Sprintf("%d,%d,%d,%d", pos[0], pos[1], pos[2], pos[3])
}
...
func to4dMap(twoSpace [][]rune) space {
	s := space{make(map[string]bool)}

	for i, row := range twoSpace {
		for j, r := range row {
			if r == '#' {
				s.points[toKey([]int{i, j, 0, 0})] = true
			}
		}
	}
	...
	for i := point[0] - 1; i <= point[0]+1; i++ {
		for j := point[1] - 1; j <= point[1]+1; j++ {
			for k := point[2] - 1; k <= point[2]+1; k++ {
				for l := point[3] - 1; l <= point[3]+1; l++ {
					if i != point[0] || j != point[1] || k != point[2] || l != point[3] {
						resp = append(resp, []int{i, j, k, l})
					}
				}
			}
		}
