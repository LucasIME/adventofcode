...
proc processValues(l: seq[int], typeId: int): int =
    case typeId:
        of 0:
            return l.foldl(a+b)
        of 1:
            return l.foldl(a*b, 1)
        of 2:
            return l.foldl(min(a,b))
        of 3:
            return l.foldl(max(a,b))
        of 5:
            return l.foldl((if a > b: 1 else : 0))
        of 6:
            return l.foldl((if a < b: 1 else : 0))
        of 7:
            return l.foldl((if a == b: 1 else : 0))
...
proc processPacket(input: seq[int], startIndex: int): ParsePacketResponse =
    ...
    if typeId == 4:
        ...
        return ParsePacketResponse(nextIndex: i+1, cumulativeValue: bitToInt(literal), shouldStop: true)

    let lenghTypeId = input[payloadStart]
    var packetVals: seq[int] = @[]
    ...
    if lenghTypeId == 0:
        ...
        while pStart - originalPStart < packetLen:
            ...
            packetVals.add(next.cumulativeValue)
            pStart = next.nextIndex

        let response = processValues(packetVals, typeId)
        return ParsePacketResponse(nextIndex: pStart, cumulativeValue: response, shouldStop:true)
