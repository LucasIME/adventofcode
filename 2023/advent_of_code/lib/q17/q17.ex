defmodule AdventOfCode.Q17 do
  ...
  def initialize_distances(grid) do
    ...
    cells = for row <- 0..last_row, col <- 0..last_col, dir <- [:up, :down, :left, :right], do: {row, col, dir}
  ...
  def find_min_distance(unvisited, distances) do
    unvisited |> MapSet.to_list() |> Enum.map(fn node -> {node, Map.get(distances, node)} end)
    |> Enum.min_by(fn {_node, distance} -> distance end)
  end

  def update_distances(distances, neigh_with_dist) do
    Enum.reduce(neigh_with_dist, distances, fn {neigh, new_dist}, acc_dist ->
      old_dist = Map.get(acc_dist, neigh)

      if new_dist < old_dist do
        Map.put(acc_dist, neigh, new_dist)
      else
        acc_dist
      end
    end)
  ...
  def get_dist_after_steps(grid, row, col, dir, cur_dist, steps) do
    ...
    if is_valid?(grid, next_row, next_col) do
      ...
      [{{next_row, next_col, dir}, new_dist} | get_dist_after_steps(grid, next_row, next_col, dir, new_dist, steps - 1)]
    else
      []
    end
  end
  ...
  def find_shortest(grid, unvisited, distances, target) do
    {{row, col, dir}, cur_dist} = find_min_distance(unvisited, distances)
    ...
    if {row, col} == target do
      ...
      updated_distances = update_distances(distances, neigh_with_dist)
      unvisited = MapSet.delete(unvisited, {row, col, dir})

      find_shortest(grid, unvisited, updated_distances, target)
    end
  end

  def min_dist(grid) do
    distances = initialize_distances(grid)
    unsivited = Map.keys(distances) |> MapSet.new()

    target = {tuple_size(grid) - 1, tuple_size(grid |> elem(0)) - 1}

    find_shortest(grid, unsivited, distances, target)
  end
