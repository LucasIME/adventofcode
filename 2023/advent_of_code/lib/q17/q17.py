...
def get_dist_after_steps(grid, row, col, cur_dir, cur_dist, steps):
  if steps == 0:
    return []
  ...
  return [(new_dist, (next_row, next_col, cur_dir))] + get_dist_after_steps(grid, next_row, next_col, cur_dir, new_dist, steps - 1)

  

def get_neighs(grid, row, col, cur_dir, cur_dist):
  possible_dirs  = get_next_possible_dirs(cur_dir)
  return list(itertools.chain.from_iterable([ get_dist_after_steps(grid, row, col, new_dir, cur_dist, 3) for new_dir in possible_dirs ]))
  # return [ get_dist_after_steps(grid, row, col, new_dir, cur_dist, 3) for new_dir in possible_dirs ]
